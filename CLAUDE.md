...
- **Make 在执行配方第一行之前就把整个配方展开完了，`$(shell)` 也一并求值。** 所以"配方第一行 touch 一下再 build"对 `VERSION ?= $(shell git describe --dirty)` 完全无效——describe 早已跑完。v0.5.0 与 v0.5.1 两次发布都因此带着 `-dirty`，而修复代码看上去完全正确。要影响 `$(shell)` 的结果，只能把动作放进同一个 `$(shell)` 里。`TestVersionRestoresGitkeepBeforeDescribing` 守这条。
