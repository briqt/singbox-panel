---
name: singbox-panel
description: 管理 singbox-panel 代理面板——用户管理（创建/启用/禁用/配额/权限）、节点管理（CRUD/配置推送/原始编辑）、订阅链接、流量统计。当用户提到代理面板、proxy panel、节点管理、用户流量、订阅链接、singbox-panel、panel.example.com 时使用。也适用于：查看谁在用代理、给某人开通/关闭权限、推送配置到节点、查看流量统计。
---
...
获取 token:
```bash
ssh $(DEPLOY_HOST) 'grep ADMIN_TOKEN /opt/singbox-panel/.env' | cut -d= -f2
```
...
# 创建节点
POST /api/nodes
Body: {"name":"name", "host":"ip", "domain":"domain.example.com", "port":22}

# 更新节点
PUT /api/nodes/{id}
Body: {"domain":"new.example.com", "enabled": true}
...
POST /api/batch/push-all
# 4. 给朋友发订阅链接
# https://panel.example.com/sub/{sub_token}
```
...
| 节点 | IP | 协议 | 域名 |
|------|-----|------|------|
| node-a | <node-ip> | Vision+Hy2+Reality | node-a.example.com |
| node-b | <node-ip> | Vision+Hy2+Reality | node-b.example.com |
| node-c | <node-ip> | Vision+Hy2+Reality | node-c.example.com |
| de | <node-ipv6> | xray (WS+Reality) | de.example.com |
| node-c | 72.11.134.212 | Reality only | — |
...
ssh $(DEPLOY_HOST) 'systemctl stop singbox-panel'
scp bin/singbox-panel $(DEPLOY_HOST):/opt/singbox-panel/singbox-panel
ssh $(DEPLOY_HOST) 'systemctl start singbox-panel'
```
